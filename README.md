...
- **Finetuning training (Unsloth):**  
  `requirements.txt` includes `unsloth` and `trl`. Unsloth uses **unsloth/GLM-OCR** (optimized for training). For GLM-OCR support you need **transformers ≥ 5.2** (Unsloth may report a dependency conflict; training has been verified with transformers 5.2).
...
4. **Train** — Use `finetuning/output/` (images, `train.txt`, `test.txt`, `labels.json`).

   **Unsloth** — `train_unsloth.py`: FastVisionModel + LoRA + UnslothVisionDataCollator. Images are loaded on demand (lazy dataset) so large `--train-size` (e.g. 8500) does not exhaust RAM. Defaults: lr 2e-4, warmup 100 steps, LoRA r=16, alpha=16. Validation on the test set at the end of each epoch; `--eval-batch-size` (default 1) avoids eval OOM on small GPUs.

   ```bash
   # From project root. Uses all samples in train.txt / test.txt (or cap with --train-size / --test-size)
   python finetuning/train/train_unsloth.py

   # 3 epochs, 4-bit to reduce VRAM
   python finetuning/train/train_unsloth.py --epochs 3 --load-in-4bit

   # Custom sizes and eval
   python finetuning/train/train_unsloth.py --train-size 500 --test-size 50 --epochs 2 --eval-steps 25 --eval-batch-size 2
   ```

   **Token counts:** To see max tokens per page in `labels.json` (for choosing `--max-length`):
   ```bash
   python finetuning/train/tokenize_labels.py
   python finetuning/train/tokenize_labels.py --percentiles
   ```
...
- **`finetuning/train/out_unsloth/`** — Training checkpoints (in `.gitignore`).
- **`*unsloth_compiled_cache/`** — Unsloth compiled modules (in `.gitignore`).
